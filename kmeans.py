...
def mean_of_points(list_of_points):
    ...
    # Initialize a list to hold the sums of each dimension
    dimension_sums = [0] * num_dimensions

    # Sum the values in each dimension
    for point in list_of_points:
        for i in range(num_dimensions):
            dimension_sums[i] += point[i]

    # Divide the sums by the number of points to get the mean
    num_points = len(list_of_points)
    mean = [dimension_sums[i] / num_points for i in range(num_dimensions)]
